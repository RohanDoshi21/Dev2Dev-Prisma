...
const changeStatus = async (req: Request, res: Response) => {
    try {
        ...
        const question = await questionService.getQuestionStatusById(parseInt(questionId));
        if (!question) {
            throw new Error("Question not found");
        }
        question.status = status;
        const updatedQuestion = await questionService.updateQuestion(parseInt(questionId), question);
